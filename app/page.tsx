...
function MainPortal({ session, onLogout, submittedDeals, setSubmittedDeals, users, setUsers, teamMembers, setTeamMembers, deleteRequests, setDeleteRequests }: {
  ...
  function handleSaveLender(form: NewLenderForm) {
    setLenderRecords((prev) => [...prev, { id: prev.length + 1, source: "Dashboard", spreadsheetRow: "—", program: form.programName, lender: form.programName, type: form.capitalType, minLoan: form.minLoan, maxLoan: form.maxLoan, maxLtv: form.maxLtv, minDscr: "1.20x", states: form.targetStates.length > 0 ? form.targetStates : ["Nationwide"], assets: form.propertyTypes, status: form.status, email: form.email, phone: form.phone, recourse: form.recourse, contactPerson: form.contactPerson, website: form.website, sponsorStates: form.sponsorStates, loanTerms: form.loanTerms.join(", "), typeOfLoans: form.typeOfLoans, programTypes: form.programTypes, typeOfLenders: form.typeOfLenders, notes: form.notes }]);

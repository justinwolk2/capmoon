...
type HybridProperty = {
  id: number;
  ownershipStatus: "Acquisition" | "Refinance" | "Permanent Loan";
  assetType: string;
  dealType: string;
...
function AddressFields({ address, onChange, inputClass }: { address: AssetAddress; onChange: (a: AssetAddress) => void; inputClass: string }) {
  ...
  return (
    <div className="md:col-span-2">
      ...
      <div className="mb-3">
        <label className="text-xs text-gray-500 mb-1 block font-medium">🔍 Search Address (auto-fills fields below)</label>
        <input
          type="text"
          ...
          onChange={async (e) => {
            ...
            try {
              ...
              if (data.features?.[0]) {
                ...
                onChange({ ...address,
                  street: (f.place_name || "").split(",")[0]?.trim() || "",
...
function AddLenderPage({ onSave, onCancel, existingLenders, inputClass, selectTriggerClass }: { onSave: (form: NewLenderForm) => void; onCancel: () => void; existingLenders: LenderRecord[]; inputClass: string; selectTriggerClass: string }) {
  ...
  const [addMode, setAddMode] = useState<"new" | "program" | null>(null);
  const [parentLender, setParentLender] = useState<LenderRecord | null>(null);
  ...
  return (
    <div>
      ...
      {matcherStep === "start" && (
        <div className="max-w-2xl">
          <div className={cardClass + " p-8"}>
            <div className="mb-1 text-xs uppercase tracking-[0.22em] text-[#c9a84c] font-bold">Deal Matcher <span className="text-[#c9a84c]">Premier</span></div>
        ...
        <div className="grid h-screen lg:grid-cols-[260px_1fr] overflow-hidden">
          <aside className="hidden lg:flex flex-col border-r border-[#c9a84c]/10 bg-[#0a1f44]">
            ...
            <nav className="space-y-1 p-4 flex-1">
              {([["matcher", "Deal Matcher Premier", Filter],
    ["deal-matcher-plus", "Deal Matcher PLUS+", Zap, "sub"], ["my-deals", "My Deals", FileSpreadsheet], ["uploads", "Upload Center", Upload]] as [string, string, any][]).map(([key, label, Icon]) => (
...
function MatcherHub({ session, submittedDeals, setSubmittedDeals, lenderRecords, teamMembers, prefillDeal, setPrefillDeal, setActiveTab, inputClass, selectTriggerClass, cardClass }: { session: any; submittedDeals: any[]; setSubmittedDeals: (d: any) => void; lenderRecords: any[]; teamMembers: any[]; prefillDeal: any; setPrefillDeal: (d: any) => void; setActiveTab: (t: string) => void; inputClass: string; selectTriggerClass: string; cardClass: string; }) {
  const [tab, setTab] = React.useState<"choose"|"classic"|"plus">("choose");
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-5">
        <button onClick={() => setTab("classic")} className="text-left rounded-2xl border-2 border-gray-200 bg-white p-6 hover:border-[#0a1f44] hover:shadow-md transition-all group">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div><div className="font-display text-xl font-bold text-[#0a1f44]">Deal Matcher <span className="text-[#c9a84c]">Premier</span></div><div className="text-xs text-gray-400">Full Platform</div></div>
          </div>
          <p className="text-sm text-gray-500 mb-4">The widest range of options. Any deal, any asset class, any capital type. Best for complex deals that need deep analysis.</p>
          <div className="flex flex-wrap gap-1.5">{["Bridge","CMBS","Mezz","JV Equity","Construction","Perm","Any Asset"].map(t => <span key={t} className="px-2 py-1 text-xs bg-gray-100 text-gray-500 rounded-full">{t}</span>)}</div>
        </button>
                ...
                <div className="space-y-6">
                  <div className={cardClass + " p-6"}>
                    ...
                    {[
                      ...
                      { key: "intern",         label: "Interns",             color: "#10b981", emoji: "🎓" },
...
export default function Home() {
  ...
  function handleSubmitDeal(deal: SubmittedDeal) {
    ...
    fetch("/api/data", {
      ...
      fetch("/api/data?type=deals").then(r => r.json()).then(d => {
        if (Array.isArray(d) && d.length > 0) setSubmittedDeals(d);
      });
    }).catch(e => console.error("Deal save failed:", e));
  }
